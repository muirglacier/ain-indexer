        ...
        view.CalculatePoolRewards(poolId, onLiquidity, beginHeight, end,
            [&](RewardType type, CTokenAmount amount, uint32_t height) {
                ...
                if (height >= eunosHeight || firstHeight != height) {
                    mnview.AddBalance(owner, amount, NULL, 0); // update owner liquidity
                }
            }
        );
        return true;
...
public:
    ...
    ~CScopeAccountReverter() {
        for (const auto& balance : balances) {
            ...
            if (amount > 0) {
                view.AddBalance(owner, {balance.first, amount}, NULL, 0);
                if (IsPoolShare) {
                ...
                view.SubBalance(owner, {balance.first, -amount}, NULL, 0);
                if (IsPoolShare) {
