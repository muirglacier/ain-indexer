...
Res ATTRIBUTES::RefundFuturesContracts(CCustomCSView &mnview, const uint32_t height, const uint32_t tokenID)
{
    ...
    for (const auto& [key, value] : userFuturesValues) {
        ...
        auto res = subView.SubBalance(*contractAddressValue, value.source, &reason, height);
        if (!res) {
            return res;
        }
        subView.Flush();
        ...
        res = addView.AddBalance(key.owner, value.source, &reason, height);
        if (!res) {
            return res;
        }
        addView.Flush();
...
Res ATTRIBUTES::RefundFuturesDUSD(CCustomCSView &mnview, const uint32_t height)
{
    ...
    for (const auto& [key, amount] : userFuturesValues) {
        ...
        auto res = subView.SubBalance(*contractAddressValue, {DCT_ID{}, amount}, &reason, height);
        if (!res) {
            return res;
        }
        subView.Flush();
        ...
        res = addView.AddBalance(key.owner, {DCT_ID{}, amount}, &reason, height);
        if (!res) {
            return res;
        }
        addView.Flush();
